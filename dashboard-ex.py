...
def predict_model(threshold, model,x):
...
def main(path_test, path_train):
    ...
    if id_input == '': #lorsque rien n'a été saisi
        ...
        with st.spinner('Chargement du score du client...'):
            ...
            dict_pred_proba = predict_model(threshold, model,x)
            pred =  dict_pred_proba["prediction"]
            proba_1 = dict_pred_proba["probabilite"]
        ...
        * En Défaut : valeur moyenne pour l'ensemble des clients en défaut\n\
        ...
        if feature_to_update != '':
            ...
            if update_val != default_value:
                ...
                dict_pred_proba = predict_model(threshold, model,x_update)
                update_pred = dict_pred_proba["prediction"]
                update_proba = dict_pred_proba["probabilite"]
